...
var SwaggerRequest = function(type, url, params, opts, successCallback, errorCallback, operation, execution) {
  ...
  if(defaultGw == 'iii'){
  // Hack by Jim Lin: overwrite 'url' variable
  var jim_scheme = url.split("/")[0].replace(":","");
  var jim_host = url.split("/")[2];
  var jim_path = url.replace(jim_scheme + "://" + jim_host,"");
  var jim_port = 0;
  ...
  this.url = gwurl + jim_scheme + "/" + jim_host + "/" + jim_port + jim_path ;
  // End of Hack
  }else{
   this.url = gwurl;
   }
  ...
  // Hack by Jim Lin: add 'APIuri' header
  if(defaultGw == 'iii'){
      params.headers["APIuri"] = url;
  }else{
      params.headers["X-uri"] = url; // For PTC's GW
  }
  // End of Hack
